...
namespace FirstWebApplication.Controllers
{
    ...
    public class PilotController : Controller
    {
        ...
        public PilotController(ApplicationDbContext context, ILogger<PilotController> logger, GeometryService geometryService)
        {
            _context = context;
            _logger = logger;
            ...
            double radiusMeters)
        {
            ...
            foreach (var obstacle in obstacles)
            {
                var coords = _geometryService.ParseWktPoint(obstacle.Location);
                if (coords == null)
                    continue;

                var distance = _geometryService.CalculateDistance(latitude, longitude, coords.Value.lat, coords.Value.lng);
